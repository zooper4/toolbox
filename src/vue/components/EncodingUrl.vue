<template>
  <div>
    <ToolPageTitle title="URL 编解码" />
    <p class="mb-4 text-gray-500">URL 编码和解码工具</p>
    <div class="mb-4">
      <label class="block mb-1">输入</label>
      <textarea v-model="input" class="form-input form-input-lg" placeholder="粘贴 URL 或文本..."></textarea>
    </div>
...
import { urlDecode, urlEncode } from '../../utils/encoding-tools.js'
...
function encode() {
  ...
  try {
    output.value = urlEncode(value)
  } catch (error) {
    output.value = `输入不合法：${error?.message || 'URL 编码失败'}`
  }
}
function decode() {
  const value = String(input.value || '')
  ...
  try {
    output.value = urlDecode(value)
  } catch (error) {
    output.value = `输入不合法：${error?.message || 'URL 解码失败'}`
  }
}
</script>
