<template>
  <div>
    ...
    <div class="grid gap-4">
      ...
      <Field label="内容"><textarea v-model="des.input" class="form-input form-input-lg" placeholder="加密：明文/Hex 字节 | 解密：Base64 密文"></textarea></Field>
    </div>
...
import { DES_KEY_HEX_LENGTH, DES_KEY_HINT, formatFromBase64, formatFromTextHex, getHexRequirementMessage, isConvertError, isEvenHex, isHexLength, isHexOutput, parseBase64Input, randomHex } from './shared/cipher-helpers.js'
...
async function runDes(enc) {
  ...
  if (!enc) {
    inputText = parseBase64Input(des.input, des.format)
